...
trait Repository<DomainClass, RepositoryClass extends Repository> {
    ...
    public static RepositoryClass query(Map search = [:]) {
        Repository repository = this.getDeclaredConstructor().newInstance()
        repository.search = (Map) search.clone()
        repository.validateAllowedFilters()
        repository.addDefaultCriteria()
        repository.buildCriteria()
    ...
    public <T> T list(Map params = [:]) {
        ...
        return (T) (params.isEmpty() ? getBuildableCriteria().list(criteria) : getBuildableCriteria().list(params, criteria))
    }
